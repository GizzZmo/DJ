...
class MockAudioTrack:
    ...
    def play(self, loops: int = 0, fade_ms: int = 0) -> bool:
        ...
        print(f"[MOCK] Playing: {self.file_path.name} (loops={loops}, fade={fade_ms}ms)")
        return True
...
class MockDJMixer:
    ...
    def initialize(self) -> bool:
        ...
        print(f"[MOCK] DJ Mixer initialized: {self.frequency}Hz, {self.channels} channels")
        return True
    ...
    def apply_crossfader(self, left_track: str, right_track: str) -> bool:
        ...
        print(f"[MOCK] Crossfader applied: {left_track}={left_volume:.2f}, {right_track}={right_volume:.2f}")
        return True
...
def test_mock_mixer():
    ...
    for track_name in tracks:
        ...
        print(f"  {track_name}: {'PLAYING' if playing else 'STOPPED'}, Volume: {volume:.2f}")
