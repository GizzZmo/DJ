...
class DJMixerGUI:
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(
            main_frame, text="AI-POWERED DJ MIXER", font=("Arial", 24, "bold")
        )
        ...
        self.init_button = ttk.Button(
            init_frame, text="Initialize Mixer", command=self.initialize_mixer
        )
        self.init_button.pack(side=tk.LEFT, padx=(0, 10))

        self.status_label = ttk.Label(
            init_frame, text="Mixer not initialized", foreground="red"
        )
        self.status_label.pack(side=tk.LEFT, padx=(0, 20))

        # AI status label
        self.ai_status_label = ttk.Label(
            init_frame, textvariable=self.ai_status_var, foreground="blue"
        )
    ...
    def create_deck_section(
        self, parent, title, row, col, file_var, status_var, vol_var, deck_name
    ):
        """Create a deck control section"""
        # Main deck frame
        deck_frame = ttk.LabelFrame(parent, text=title, padding="10")
        deck_frame.grid(
            row=row, column=col, padx=5, pady=5, sticky=(tk.W, tk.E, tk.N, tk.S)
        )
        ...
        play_button = ttk.Button(
            control_frame, text="Play", command=lambda: self.play_track(deck_name)
        )
        play_button.pack(side=tk.LEFT, padx=(0, 5), fill=tk.X, expand=True)

        pause_button = ttk.Button(
            control_frame, text="Pause", command=lambda: self.pause_track(deck_name)
        )
        pause_button.pack(side=tk.LEFT, padx=(0, 5), fill=tk.X, expand=True)

        stop_button = ttk.Button(
            control_frame, text="Stop", command=lambda: self.stop_track(deck_name)
        )
        ...
        status_label = ttk.Label(
            status_frame, textvariable=status_var, font=("Arial", 10, "bold")
        )
    ...
    def create_crossfader_section(self, parent, row, col):
        ...
        apply_button = ttk.Button(
            cross_frame, text="Apply Crossfader", command=self.apply_crossfader
        )
        apply_button.pack(fill=tk.X)

        # Instructions
        instruction_text = (
            "Crossfader controls the balance between Deck 1 (left) and Deck 2 (right)"
        )
    ...
    def create_master_section(self, parent, row, col):
        ...
        master_frame.grid(
            row=row, column=col, columnspan=3, padx=5, pady=5, sticky=(tk.W, tk.E)
        )
        ...
        self.master_vol_var.trace(
            "w",
            lambda *args: self.master_vol_label.config(
                text=f"{self.master_vol_var.get():.2f}"
            ),
        )
    ...
    def create_ai_section(self, parent, row, col):
        ...
        key_entry = ttk.Entry(
            key_frame, textvariable=self.gemini_api_key, show="*", width=30
        )
        key_entry.pack(side=tk.LEFT, padx=(5, 0), fill=tk.X, expand=True)

        configure_button = ttk.Button(
            key_frame, text="Configure AI", command=self.configure_ai
        )
        ...
        self.auto_mix_button = ttk.Button(
            auto_mix_frame, text="Start Auto Mix", command=self.start_auto_mix
        )
        self.auto_mix_button.pack(fill=tk.X, pady=(0, 2))

        self.stop_auto_mix_button = ttk.Button(
            auto_mix_frame,
            text="Stop Auto Mix",
            command=self.stop_auto_mix,
            state=tk.DISABLED,
        )
        ...
        self.analyze_keys_button = ttk.Button(
            key_mix_frame, text="Analyze Keys", command=self.analyze_track_keys
        )
        self.analyze_keys_button.pack(fill=tk.X, pady=(0, 2))

        self.key_advice_button = ttk.Button(
            key_mix_frame, text="Get Key Advice", command=self.get_key_mixing_advice
        )
        self.key_advice_button.pack(fill=tk.X)

        # Fader effects controls
        effects_frame = ttk.LabelFrame(
            controls_frame, text="Fader Effects", padding="5"
        )
        effects_frame.pack(side=tk.LEFT, fill=tk.X, expand=True)

        self.fader_effects_button = ttk.Button(
            effects_frame, text="Suggest Effects", command=self.suggest_fader_effects
        )
        self.fader_effects_button.pack(fill=tk.X, pady=(0, 2))

        self.apply_effects_button = ttk.Button(
            effects_frame, text="Apply Effects", command=self.apply_ai_effects
        )
        self.apply_effects_button.pack(fill=tk.X)

        # AI Info display
        self.ai_info_text = tk.Text(
            ai_frame, height=4, width=80, state=tk.DISABLED, wrap=tk.WORD
        )
        self.ai_info_text.pack(fill=tk.X, pady=(10, 0))

        # Initialize AI info
        self.update_ai_info(
            "Ready to configure AI assistant. Enter your Gemini API key above."
        )
    ...
    def configure_ai(self):
        ...
        if success:
            ...
            self.update_ai_info(
                "🤖 AI Assistant configured successfully! Ready for intelligent mixing."
            )
        else:
    ...
    def start_auto_mix(self):
        ...
        self.update_ai_info(
            "🎛️ AI Auto Mix active - analyzing tracks and optimizing transitions..."
        )
    ...
    def analyze_track_keys(self):
        ...
        for track in loaded_tracks:
            track_file = (
                self.deck1_file.get() if track == "deck1" else self.deck2_file.get()
            )
            if track_file != "No file loaded":
                analysis = self.ai_assistant.analyze_track(track, track_file)
                analysis_results.append(
                    f"{track.upper()}: {analysis.key} ({analysis.genre}, {analysis.tempo} BPM)"
                )
                self.log_message(
                    f"📊 {track.upper()}: Key={analysis.key}, Tempo={analysis.tempo}, Energy={analysis.energy}"
                )
...
🎛️ Suggested Action: {advice['suggested_action']}"""
        ...
        self.update_ai_info(info)
        self.log_message(
            f"🎹 Key compatibility: {advice['compatibility']} ({advice['deck1_key']} + {advice['deck2_key']})"
        )
    ...
    def suggest_fader_effects(self):
        ...
        if not deck1_analysis or not deck2_analysis:
            messagebox.showinfo(
                "Info", "Please analyze track keys first to get energy levels"
            )
            return
...
💡 {advice.reasoning}"""
    ...
    def create_status_section(self, parent, row, col):
        ...
        self.status_text = tk.Text(
            status_frame, height=8, width=80, state=tk.DISABLED, wrap=tk.WORD
        )
    ...
    def initialize_mixer(self):
        ...
        if self.mixer.initialize():
            ...
            self.status_label.config(
                text="Mixer initialized successfully", foreground="green"
            )
    ...
    def load_track(self, deck_name, file_var):
        ...
        filename = filedialog.askopenfilename(
            title=f"Load track for {deck_name.upper()}", filetypes=filetypes
        )
        ...
        if filename:
            if self.mixer.load_track(deck_name, filename):
                ...
                self.log_message(
                    f"✓ Loaded {Path(filename).name} into {deck_name.upper()}"
                )
                ...
                if self.ai_assistant.is_configured:
                    ...
                    self.log_message(
                        f"🤖 AI analysis completed for {deck_name.upper()}"
                    )
            else:
                messagebox.showerror(
                    "Error", f"Failed to load track into {deck_name.upper()}"
                )
    ...
    def play_track(self, deck_name):
        ...
        if self.mixer.play_track(deck_name):
            ...
            messagebox.showwarning(
                "Warning", f"Cannot play {deck_name.upper()} - track not loaded"
            )
