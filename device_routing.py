...
class AudioDeviceManager:
    ...
    def add_route(
        self, source: str, device_index: int, channels: Optional[List[int]] = None
    ) -> bool:
        """Add an audio route"""
        ...
        route = DeviceRoute(
            source=source, device_index=device_index, channels=channels, enabled=True
        )
        ...
        if self.use_mock:
            print(
                f"[MOCK] Route added: {source} -> {device.name} (channels {channels})"
            )
...
def demo_device_routing():
    ...
    for device in devices:
        ...
        print(
            f"{device.index}: {device.name} ({device.max_output_channels} ch){default_marker}"
        )
    ...
    for route in info["routes"]:
        ...
        print(
            f"  {status} {route['source']} -> {route['device']} (ch {route['channels']})"
        )
